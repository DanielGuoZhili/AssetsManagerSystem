...
import { Card, Typography, Row, Col, Space, Button, Modal, Form, Input, InputNumber, message, Divider, Tag, Statistic } from 'antd'
import { 
  EditOutlined, 
  SaveOutlined, 
  DollarOutlined,
...
const defaultData: FinanceData = {
  subscriptions: [
    ...
    { id: '2', name: 'iCloud 450', amount: '450', cycle: '每月', currency: '¥' },
    { id: '3', name: 'Cursor', amount: '20', cycle: '每月', currency: '$' },
    { id: '4', name: 'Amazon', amount: '未知', cycle: '每年', currency: '¥' },
    { id: '5', name: 'DC 卡（JAL 会费）', amount: '未知', cycle: '每年', currency: '¥' },
  ],
  investments: [
    { id: '1', category: 'A股', name: '红利ETF', amount: '根据剩余工资', cycle: '灵活', note: 'SBI' },
    { id: '2', category: 'NISA', name: 'eMAXIS Slim 全世界株式', amount: '20000', cycle: '每月' },
    { id: '3', category: 'NISA', name: '任天堂等个股', amount: '10000', cycle: '每月', note: '也可能投入当月余钱' },
  ],
  expenses: [
    { id: '1', name: '房租', amount: '85000', cycle: '每月' },
    { id: '2', name: '年金', amount: '17100', cycle: '每月', note: '左右' },
    { id: '3', name: '保险', amount: '44000', cycle: '每月', note: '左右' },
    { id: '4', name: '住民税', amount: '71000', cycle: '每3个月', note: '一期' },
    { id: '5', name: '信用卡还款', amount: '80000-130000', cycle: '每月', note: '包括水电费、日常消费、网购' },
    { id: '6', name: '电脑分期', amount: '4900', cycle: '每月', note: '到2025年2月结束' },
...
const FinanceInfo = () => {
  ...
  const [editType, setEditType] = useState<'subscription' | 'investment' | 'expense' | 'income'>('subscription')
  ...
  const handleSave = () => {
    form.validateFields().then(values => {
      if (editType === 'income') {
        ...
        const key = editType === 'subscription' ? 'subscriptions' : 
                    editType === 'investment' ? 'investments' : 'expenses'
        
        if (editingItem) {
          // 编辑现有项
          const newItems = data[key].map((item: any) => 
            item.id === editingItem.id ? { ...values, id: editingItem.id } : item
          )
  ...
  // 删除项目
  const handleDelete = (type: 'subscription' | 'investment' | 'expense', id: string) => {
    Modal.confirm({
      title: '确认删除',
      content: '确定要删除这个项目吗？',
      onOk: () => {
        const key = type === 'subscription' ? 'subscriptions' : 
                    type === 'investment' ? 'investments' : 'expenses'
  ...
  return (
    <div className="finance-info">
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
              ...
              <Col xs={24} sm={12} lg={8} key={sub.id}>
                ...
                >
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    <Text type="secondary">
                      {sub.currency}{sub.amount} / {sub.cycle}
                    </Text>
                  </Space>
                </Card>
              </Col>
                    ...
                    <Col xs={24} sm={12} lg={8} key={inv.id}>
                      ...
                      >
                        <Space direction="vertical" style={{ width: '100%' }}>
                          ...
                          <Text>¥{inv.amount} / {inv.cycle}</Text>
                          {inv.note && <Text type="secondary" style={{ fontSize: 12 }}>{inv.note}</Text>}
                        </Space>
                      </Card>
                    </Col>
                  ))}
                ...
                >
                  <Space direction="vertical" style={{ width: '100%' }}>
                    ...
                    <Space size="small">
                      ...
                      {expense.note && <Text type="secondary" style={{ fontSize: 12 }}>{expense.note}</Text>}
                    </Space>
                  </Space>
                </Card>
              </Col>
        ...
        <Card>
          <Space direction="vertical">
            ...
            <Text type="secondary">• 预计月支出会自动计算（含周期性支出的月均值）</Text>
          </Space>
        </Card>
      </Space>
      ...
      <Modal
        title={
          editType === 'income' ? '编辑收入' :
          editType === 'subscription' ? (editingItem ? '编辑订阅' : '添加订阅') :
          editType === 'investment' ? (editingItem ? '编辑投资' : '添加投资') :
          (editingItem ? '编辑支出' : '添加支出')
        }
